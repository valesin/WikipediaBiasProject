...
* **Attribute enrichment and transformation** – Node attributes are merged and transformed: `bigperiod_birth` is collapsed into five bins and `un_subregion` is collapsed into `Western`/`Non‑Western` (`analysis.py`, lines 864‑188). Rows with NaNs for the analysed attribute are dropped *per attribute* (`analysis.py`, lines 507‑516).  
