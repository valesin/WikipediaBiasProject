...
def doubly_stochastic(table, undirected=False, return_self_loops=False):
    ...
    table = (
        pd.pivot_table(
            table, values="nij", index="src", columns="trg", aggfunc="sum", fill_value=0
        )
        + 0.001
    )
    ...
    if undirected:
        ...
        while (
            ...
            or nx.number_of_nodes(G) < original_nodes
        ):
            edge = table.iloc[i]
        ...
        G = nx.DiGraph()
        while (
            nx.number_weakly_connected_components(G) != 1
            or nx.number_of_nodes(G) < original_nodes
        ):
            edge = table.iloc[i]
            G.add_edge(edge["src"], edge["trg"], weight=edge["value"])
    ...
    table.rename(
        columns={"index": "src", "variable": "trg", "value": "cij"}, inplace=True
    )
...
def disparity_filter(table, undirected=False, return_self_loops=False):
    ...
    table["variance"] = (table["trg_count"] ** 2) * (
        (
            (20 + (4.0 * table["trg_count"]))
            / (
                (table["trg_count"] + 1.0)
                * (table["trg_count"] + 2)
                * (table["trg_count"] + 3)
    ...
    if undirected:
        table["edge"] = table.apply(
            lambda x: "%s-%s" % (min(x["src"], x["trg"]), max(x["src"], x["trg"])),
            axis=1,
        )
...
def maximum_spanning_tree(table, undirected=False):
    ...
    table["distance"] = 1.0 / table["nij"]
    G = nx.from_pandas_edgelist(
        table, source="src", target="trg", edge_attr=["distance", "nij"]
    )
    T = nx.minimum_spanning_tree(G, weight="distance")
    table2 = nx.to_pandas_edgelist(T)
    table2 = table2[table2["nij"] > 0]
    table2.rename(
        columns={"source": "src", "target": "trg", "nij": "score"}, inplace=True
    )
